...
//These data are precaptched.
static uint8_t ipudptemp[]={0x45,0x00,0x01,0x75,0x1f,0x8d,0x00,0x00,0x01,0x11,0xe7,0x47,0xc0,0xa8,0x01,0x01,0xef,0xff,0xff,0xfa};

//This function make a 'Magic Packet' which contains Wake On Lan instruction within its UDP payload.
void make_wol_on_ip(uint8_t* buf,uint8_t *dmac)
{ 
        uint8_t i=0,j=0;
		uint16_t datalen=102;
		uint16_t ck;
        while(i<6){
                buf[ETH_DST_MAC +i]=dmac[i];
                buf[ETH_SRC_MAC +i]=macaddr[i];
                i++;
        }
		i=0;
		...
		buf[ETH_TYPE_L_P]=ETHTYPE_IP_L_V;
		i=0;
		while(i<sizeof(ipudptemp))
		{
			buf[0x0E+i]=ipudptemp[i];
			i++;
		}
        while(i<4){
                buf[IP_DST_P+i]=255;
        ...
        buf[UDP_SRC_PORT_L_P]=0xFF;
		buf[UDP_DST_PORT_H_P]=0;
        buf[UDP_DST_PORT_L_P]=0;
        buf[UDP_LEN_H_P]=0;
        buf[UDP_LEN_L_P]=UDP_HEADER_LEN+datalen;
        // zero the checksum
        buf[UDP_CHECKSUM_H_P]=0;
